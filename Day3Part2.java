...
//        URL path =  Main.class.getResource("tests.txt");
        ...
        ArrayList<Triple> collisions = new ArrayList<Triple>();
        ...
        for(int i = 0; i < firstPath.length(); i++){
            ...
            while(num > 0){
                Triple temp = new Triple(x, y, count);
                path.add(temp);
                num--;
                count++;
                if(direction == 'U'){
        ...
        for(int i = 0; i < secondPath.length(); i++) {
            ...
            while(num > 0){
                for(int j = 0; j < path.size(); j++){
                    Triple temp = path.get(j);
                    if((temp.x == x) && (temp.y == y) && (x != 0) && (y != 0)){
                        Triple coll = new Triple(x, y, count+temp.time);
                        collisions.add(coll);
                    }
        ...
        x = 0;
        y = 0;
        count = collisions.get(0).time;
        int shortest = abs(collisions.get(0).x) + abs(collisions.get(0).y);
        for(int i = 0; i < collisions.size(); i++){
            if(collisions.get(i).time < count) {
                count = collisions.get(i).time;
